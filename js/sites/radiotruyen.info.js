class RadioTruyen extends Site {
  static {
    this.domain = "radiotruyen.info";
    this.category_page_detect = () => { return window.location.href.includes("/the-loai/"); };
    this.post_page_detect = ".portfolio-article";
  ...
  static craw() {
    if (this.isCategory()) {
      ...
      console.log('data_author', data_author);
      ...
      //title
      let title = $('.article-body-wrap h1').text();
      if (typeof title == 'undefined') {
        title = '';
      }
      //post image
      let post_image = $('figure.media-wrap img').attr('src');
      post_image = 'https://radiotruyen.info' + post_image;
      post_image = post_image.replace('?v=1', '').trim();
